...
def msg_confirmacao_pesq(nome, rg, cpf, acao_realizada, unidade_cons):
    texto = f"""
    Eu, {nome}, portador do RG n° {rg} e CPF n° {cpf}, pesquisador responsável pela execução do projeto intitulado "{acao_realizada}" a ser realizado na Unidade de Conservação "{unidade_cons}", assumo o compromisso junto ao Departamento de Unidades de Conservação/Secretaria de Estado do Meio Ambiente (DEUC/SEMA) de cumprir as obrigações abaixo listadas:

    1. Repassar informações à DEUC/SEMA sobre o referido projeto de minha responsabilidade, na forma de tese, dissertação, monografia, artigo, relatório parcial, relatório final, registro fotográfico, entre outros, conforme compromisso assumido no ato da solicitação, sob pena das sanções previstas em lei;
